...
def fragmento_mapa():
    ...
    # --- Selección por figuras dibujadas (CORREGIDO PARA EVITAR BUCLES EN ZOOM/PAN) ---
    dibujos_actuales = salida_mapa.get("all_drawings") or []
    # Comparamos por CANTIDAD de dibujos o si hay un dibujo activo nuevo.
    # Esto evita que los cambios de formato de flotantes de Folium durante el zoom/pan disparen un rerun falso.
    if len(dibujos_actuales) != len(st.session_state.geometrias_dibujadas) or salida_mapa.get("last_active_drawing"):
        if dibujos_actuales != st.session_state.geometrias_dibujadas:
            st.session_state.geometrias_dibujadas = dibujos_actuales
            aplicar_seleccion_por_dibujos(dibujos_actuales)
            rerun_app()

    # --- Selección por click individual sobre un punto (CORREGIDO CON FIRMA DE CLIC) ---
    tooltip_clickeado = salida_mapa.get("last_object_clicked_tooltip")
    last_clicked_coords = salida_mapa.get("last_clicked")
    
    click_signature = (tooltip_clickeado, json.dumps(last_clicked_coords)) if tooltip_clickeado and last_clicked_coords else None

    if click_signature and click_signature != st.session_state.ultimo_click_procesado:
        st.session_state.ultimo_click_procesado = click_signature
